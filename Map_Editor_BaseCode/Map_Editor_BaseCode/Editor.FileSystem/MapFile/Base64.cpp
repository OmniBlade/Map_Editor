...
bool base64_initialized = false;
unsigned char base64_map[0x100] = {};
...
void base64_initmap() {
	if (!base64_initialized) {
		std::fill(std::begin(base64_map), std::end(base64_map), 0xFF);
...
{
	std::string pack;
	int allocSize = section->totalSize();
	pack.reserve(allocSize);
